...
if ($tmdb_movie_id <= 0 || !in_array($status, $valid_statuses, true)) {
    http_response_code(400);
    echo json_encode(array('success' => false, 'error' => 'Invalid movie status.'));
    mysqli_close($conn);
    exit;
}
...
$stmt = mysqli_prepare(
    $conn,
    "INSERT INTO MRS_UserMovieList (account_id, tmdb_movie_id, status)
     VALUES (?, ?, ?)
...
mysqli_stmt_bind_param($stmt, "iis", $account_id, $tmdb_movie_id, $status);

if (mysqli_stmt_execute($stmt)) {
    echo json_encode(array('success' => true, 'status' => $status));
} else {
    http_response_code(500);
    echo json_encode(array('success' => false, 'error' => 'Unable to save movie status.'));
}
